...
from .utils import ensure_extension, geocode_cache_key, normalise_session_id, safe_project_name, to_dms
...
class QgisLatLonPlugin:
    ...
    def initGui(self):
        ...
        self.reverse_geocode_action = QAction("Enable reverse geocoding (OpenStreetMap)", parent)
        ...
        self.reverse_geocode_action.toggled.connect(
            lambda enabled: self.settings.setValue(f"{SETTINGS_PREFIX}/reverse_geocode", enabled)
        )
    ...
    def _menu_actions(self) -> List[QAction]:
        return [action for action in (
            self.capture_action, self.panel_action, self.copy_action, self.reverse_geocode_action
    ...
    def activate(self, checked: bool = True):
        ...
        self.iface.messageBar().pushMessage(
            "GeoClick Capture", "Capture mode is active. Click the map to record a point.",
    ...
    def handle_map_click(self, map_point: QgsPointXY, _button):
        ...
        feature_id = self.add_point_to_layer(
            ...
            project_name=safe_project_name(project.fileName(), project.title() or "Untitled project"),
    ...
    def _identify_source(self, map_point: QgsPointXY) -> Tuple[str, str, str]:
        ...
        try:
            ...
            results = identify.identify(
                int(canvas_point.x()), int(canvas_point.y()), layers,
                QgsMapToolIdentify.TopDownStopAtFirst,
            )
        if not results:
            return "", "", ""
    ...
    def _prepare_layer(self) -> bool:
        ...
        missing = [QgsField(name, field_type) for name, field_type in FIELD_DEFINITIONS
    ...
    def add_point_to_layer(self, map_point, wgs84_point, captured_at, project_authid,
                           project_name, map_scale, source_layer, source_layer_id,
                           source_feature_id, location, context) -> Optional[int]:
    ...
    def clear_session(self):
        ...
        answer = QMessageBox.question(
            self.iface.mainWindow(), "GeoClick Capture",
    ...
    def _queue_reverse_geocode(self, lat: float, lon: float, feature_id: int):
        ...
        if key in self._geocode_cache:
            ...
            return
        wait_ms = max(0, int((1.0 - (time.monotonic() - self._last_geocode_request)) * 1000))
        QTimer.singleShot(wait_ms, lambda: self._request_reverse_geocode(lat, lon, feature_id, key))

    def _request_reverse_geocode(self, lat, lon, feature_id, key):
        self._last_geocode_request = time.monotonic()
        parameters = urlencode({
            "lat": f"{lat:.8f}", "lon": f"{lon:.8f}", "format": "jsonv2",
            "zoom": 18, "addressdetails": 0,
        })
        request = QNetworkRequest(QUrl(f"https://nominatim.openstreetmap.org/reverse?{parameters}"))
        request.setRawHeader(
            b"User-Agent", b"GeoClick-Capture/1.2 (+https://github.com/Jubilio/qgis-latlon)"
        )
        ...
        reply.finished.connect(
            lambda r=reply, fid=feature_id, cache_key=key:
            self._finish_reverse_geocode(r, fid, cache_key)
        )

    def _finish_reverse_geocode(self, reply, feature_id: int, cache_key: str):
        try:
            if reply.error():
                self.iface.messageBar().pushWarning("GeoClick Capture", "Reverse geocoding is unavailable.")
                return
            ...
            if location:
                ...
                self.iface.messageBar().pushMessage(
                    "Place identified", f"{location} — © OpenStreetMap contributors",
                    level=Qgis.Info, duration=7,
                )
        except (ValueError, UnicodeDecodeError, RuntimeError) as exc:
            self.iface.messageBar().pushWarning("GeoClick Capture", f"Invalid geocoding response: {exc}")
        finally:
    ...
    def _apply_location(self, feature_id: int, location: str):
        ...
        if index >= 0:
            self.layer.dataProvider().changeAttributeValues({feature_id: {index: location}})
            self.layer.triggerRepaint()
            self._refresh_dock()
    ...
    def copy_to_clipboard(self):
        ...
        self.iface.messageBar().pushMessage("Copied", text, level=Qgis.Success, duration=4)
    ...
    def _export_csv(self):
        path, _ = QFileDialog.getSaveFileName(self.iface.mainWindow(), "Save as CSV", "", "CSV (*.csv)")
        if not path:
            return
    ...
    def _export_vector(self, driver: str, extension: str, file_filter: str):
        path, _ = QFileDialog.getSaveFileName(self.iface.mainWindow(), "Save layer", "", file_filter)
        if not path:
            return
    ...
    def _load_preferences(self):
        return {key: self.settings.value(f"{SETTINGS_PREFIX}/{key}", default)
                for key, default in (
                    ("session_id", ""), ("operator", ""),
                    ("category", "General"), ("status", "Unreviewed"),
                )}
    ...
    def _success(self, message: str):
        self.iface.messageBar().pushMessage("GeoClick Capture", message, level=Qgis.Success, duration=6)
